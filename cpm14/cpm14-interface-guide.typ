...
#let document-version = [version 2025-08-06]
...
#figure(
  table(
    ...
    `FBASE:`
    #footnote[Note: the exact addresses for `BOOT`, `TBASE`, `CBASE`, `FBASE`,
  and `ENTRY` vary with the CP/M version. (See @AddressAssignments for version correspondence).]
  ],
...
are loaded into the TPA and executed as
...
`TBASE` in memory(see the CP/M `LOAD` command).
The CCP loads the `COM` file from the disk into
memory starting at `TBASE` and can extend up to `CBASE`.

If the `<command>` is followed by either a `<filename>` or
`<filename>.<filetype>`, then the
CCP prepares a File Control Block (FCB) names in the
system parameter area. 
This FCB is in the form required to access the file 
through the FDOS and are described in
@OSConventions.

The program then executes, perhaps using the I/O facilities of the FDOS
If the program uses no FDOS facilities,
then the entire remaining memory area is available for data used by the program.
If the FDOS is to remain in memory, then the transient program can use only
up to the location `FBASE` as data. #footnote[The address `ENTRY` contains a jump to the lowest
address in the FDOS, and the `ENTRY+1` contains the first FDOS address which cannot
be overlayed.]

The transient uses the CP/M I/O facilities to
communicate with the operator's console and peripheral devices,
including the floppy disk subsystem. The I/O system is accessed by
passing a "function number" and an "information address" to CP/M
through the FDOS entry point at `ENTRY`. In the case of a disk
...
The function numbers and error indicators are given in detail in @TableI.
...
#bdos-function-summary-table(
  "1.4",
  caption: "CP/M 1.4 Function Summary",
  numbering: "1"
) <TableI>
...
- #link("https://www.icl1900.co.uk/unix4fun/z80pack/ftp/manuals/DRI/cpm-1.4/CPM_1.4_Interface_Guide.pdf")[_CP/M 1.4 Interface Guide_ (PDF)]
